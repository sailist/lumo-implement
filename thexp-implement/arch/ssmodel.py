...
class ContrastiveModel(nn.Module):
    def __init__(self, backbone, backbone_dim, head='mlp', feature_dim=128, n_classes=10):
        ...
        if head == 'linear':
            ...
            self.contrastive_head = nn.Sequential(
                ...
                nn.ReLU(), nn.Linear(self.backbone_dim, feature_dim))
        else:
            raise ValueError('Invalid head {}'.format(head))
        self.fc = nn.Sequential(self.contrastive_head,
...
class ClusteringModel(nn.Module):
    def __init__(self, backbone, feature_dim, nclusters, nheads=1):
        super(ClusteringModel, self).__init__()
        self.backbone = backbone
        self.backbone_dim = feature_dim
        self.nheads = nheads
