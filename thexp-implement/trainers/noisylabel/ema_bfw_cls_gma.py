...
@deprecated，似乎不好用，建模时候可能离得太近了，导致没有办法区分（可能还是数据量的问题）
使用 BMM 对每个类建模区分
cifar10
0001.371e765a，0.2
0004.904fcff0，0.8，88.40%

cifar100
0005.ae78b400，0.8，38.72% 不行的
"""
...
class MultiHeadTrainer(datasets.SyntheticNoisyMixin,
    ...
    def train_batch(self, eidx, idx, global_step, batch_data, params: NoisyParams, device: torch.device):
        ...
        if eidx == 1:
            self.cls_mem[ids, 0] = nys
        elif eidx == 2:
            self.cls_mem[ids, 1] = ys
        else:
    ...
    def on_train_epoch_end(self, trainer: 'NoisyTrainer', func, params: NoisyParams, meter: Meter, *args, **kwargs):
        if params.eidx == 1:
            nys = self.cls_mem[:, 0].detach().cpu().numpy()
            self.nys_ids = []
            for i in range(params.n_classes):
                self.nys_ids.append(np.where(nys == i)[0])

        with torch.no_grad():
            f_mean = self.false_pred_mem[:, max(params.eidx - params.mix_burnin, 0):params.eidx].mean(
                dim=1).cpu().numpy()
