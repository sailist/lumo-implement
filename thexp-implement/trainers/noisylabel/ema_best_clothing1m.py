...
class GmaParams(NoisyParams):
    ...
    def __init__(self):
        ...
        self.cut_size = 7168  # 7168，10752  bs * 64
        self.n_classes = 14
    ...
    def initial(self):
        ...
        self.lr_sche = self.SCHE.List([
            self.SCHE.Log(lr1, lr2, right=25),
...
class MultiHeadTrainer(datasets.Clothing1mDatasetMixin,
    ...
    def train_batch(self, eidx, idx, global_step, batch_data, params: GmaParams, device: torch.device):
        ...
        with torch.no_grad():
            ...
            false_pred = raw_targets.gather(1, nys.unsqueeze(dim=1)).squeeze()  # [nys != nys]
    ...
    def on_train_epoch_end(self, trainer: 'NoisyTrainer', func, params: GmaParams, meter: Meter, *args, **kwargs):
        ...
        with torch.no_grad():
            ...
            noisy_cls = self.gmm_predict(feature)  # , mean=params.feature_mean, offset=params.offset_sche(params.eidx))
