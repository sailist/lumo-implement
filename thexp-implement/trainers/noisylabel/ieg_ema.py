...
class IEGTrainer(datasets.IEGSyntheticNoisyMixin,
    ...
    def train_batch(self, eidx, idx, global_step, batch_data, params: IEGParams, device: torch.device):
        ...
        if eidx >= params.burnin:
            ...
            fweight[weight <= 0] -= params.loc_sche(eidx)
        ...
        meter.tw = self.noisy_cls[ids][ys == nys].mean()
        meter.fw = self.noisy_cls[ids][ys != nys].mean()
        meter.tp = (weight <= 0)[ys == nys].float().mean()
        meter.fp = (weight <= 0)[ys != nys].float().mean()
