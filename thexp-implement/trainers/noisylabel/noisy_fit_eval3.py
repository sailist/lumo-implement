"""
混合干净标签的噪音样本，当模型在干净样本准确率达到最大（此时噪音样本准确率最低）的情况下，
是否实际上模型已经学到了噪音标签的pattern，只是没有在 logits 上表现出来？

验证内容：
    a. 当混合噪音样本训练到准确率最高时，拟合第二批噪音样本的用时。
    b. 对比实验：相应的完全干净的样本训练到完全拟合以及训练到相应准确率时，拟合第二批噪音样本的用时。
