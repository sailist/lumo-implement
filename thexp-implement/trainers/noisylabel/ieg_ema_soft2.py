...
class IEGParams(NoisyParams):
    ...
    def initial(self):
        ...
        self.lr_sche = self.SCHE.Cos(start=self.optim.args.lr, end=0.000001, left=0, right=params.epoch - 50)
...
class IEGTrainer(datasets.IEGSyntheticNoisyMixin,
    ...
    def train_batch(self, eidx, idx, global_step, batch_data, params: IEGParams, device: torch.device):
        ...
        meter.pm = mask.mean()
        smoothed_ntargets = tricks.label_smoothing(tricks.onehot(nys, params.n_classes))
        smoothed_ptargets = tricks.label_smoothing(tricks.onehot(p_labels, params.n_classes))
        meter.Lall = meter.Lall + self.loss_ce_with_targets_masked_(logits, smoothed_ntargets, fweight,
                                                                    meter=meter)

        meter.Lall = meter.Lall + self.loss_ce_with_targets_masked_(logits, smoothed_ptargets,
                                                                    (1 - fweight) * mask,
                                                                    meter=meter,
