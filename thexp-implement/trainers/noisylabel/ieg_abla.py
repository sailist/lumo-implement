...
train checkpoints : 0001.37942f47
ablation study of IEG
"""
...
class IEGTrainer(datasets.IEGSyntheticNoisyMixin,
    ...
    def callbacks(self, params: IEGParams):
        ...
        callbacks.callbacks.TimingCheckpoint(50).hook(self)
    ...
    def meta_optimizer(self,
        ...
        # weight_0 = torch.zeros(guess_targets.shape[0],
        #                        dtype=torch.float,
        #                        device=self.device)
        ...
        mixed_labels = tricks.elementwise_mul(eps, n_targets) + tricks.elementwise_mul(1 - eps, guess_targets)
    ...
    def train_batch(self, eidx, idx, global_step, batch_data, params: IEGParams, device: torch.device):
        ...
        meter.tww = weight[ys == nys].mean()
        meter.fww = weight[ys != nys].mean()
        ...
        meter.Llamda = -torch.mean(torch.sum(F.log_softmax(logits, dim=1) * init_mixed_labels, dim=1)) * 0.01  # Lλ*
        # meter.Llamda = 0  # Lλ*
...
if __name__ == '__main__':
    ...
    #     '/home/share/yanghaozhe/experiments/thexp-implement2.19/ieg_abla.noisylabel/0001.37942f47/modules/keep.0000150.ckpt')
    trainer.train()
