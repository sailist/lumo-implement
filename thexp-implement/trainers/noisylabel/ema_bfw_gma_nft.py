...
class GmaParams(NoisyParams):
    ...
    def initial(self):
        super().initial()
        self.filter_ema_sche = self.SCHE.Log(start=0.999, end=0.999, right=self.epoch)
        ...
        self.offset_sche = self.SCHE.Cos(start=0,
                                         ...
                                         right=self.epoch // 2)
        self.erl_sche = self.SCHE.Exp(start=0.5, end=1, right=self.epoch // 2)
...
class MultiHeadTrainer(datasets.SyntheticNoisyMixin,
    ...
    def train_batch(self, eidx, idx, global_step, batch_data, params: GmaParams, device: torch.device):
        ...
        update_mask = self.count[ids] < params.eidx
        ...
        false_pred = raw_targets.gather(1, nys.unsqueeze(dim=1)).squeeze()  # [ys != nys]
    ...
    def on_train_epoch_end(self, trainer: 'NoisyTrainer', func, params: GmaParams, meter: Meter, *args, **kwargs):
        ...
        force_mask = self.count < params.eidx
        # self.true_pred_mem[force_mask, params.eidx - 1] = self.true_pred_mem[force_mask, params.eidx - 2]
        # self.false_pred_mem[force_mask, params.eidx - 1] = self.false_pred_mem[force_mask, params.eidx - 2]
        # self.loss_mem[force_mask, params.eidx - 1] = self.loss_mem[force_mask, params.eidx - 2]
        self.count[force_mask] += 1
        self.logger.info('pvalue max', self.plabel_mem.max())
        self.logger.info(self.count.float().mean())
        with torch.no_grad():
            ...
            if params.eidx > 1:
                ...
                # _sort_idx = self.filter_mem.argsort(descending=True)
                # _first_idx = torch.where(self.filter_mem > params.erl_sche(params.eidx))[0]
                # _sec_idx = torch.where(self.filter_mem < params.erl_sche(params.eidx))[0]
                #
                # _sort_idx = torch.cat([_first_idx, _first_idx, _sec_idx])
                #
                # _sort_idx = _sort_idx.cpu().numpy()
                #
                # self.train_set.subset(_sort_idx)
                # self.logger.info('set acc', true_cls[_first_idx.cpu().numpy()].mean(),
                #                  'in', len(_first_idx),
                #                  'c', true_cls[_first_idx.cpu().numpy()].sum())
...
if __name__ == '__main__':
    ...
    memory(3500, device=params.device).immediately()
