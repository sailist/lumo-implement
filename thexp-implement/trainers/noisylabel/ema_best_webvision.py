...
class GmaParams(NoisyParams):
    ...
    def initial(self):
        super().initial()
        self.filter_ema_sche = self.SCHE.Log(start=0.999, end=0.99, right=self.epoch)
...
class MultiHeadTrainer(datasets.WebVisionDatasetMixin,
    ...
    def train_batch(self, eidx, idx, global_step, batch_data, params: GmaParams, device: torch.device):
        ...
        if fmask.any():
            ...
            meter.Lall = meter.Lall + self.loss_ce_with_targets_(logits[fmask], n_targets[fmask],
                                                                 # fweight[fweight > 0.5].float(),
                                                                 meter=meter)
            # meter.Lall = meter.Lall + self.loss_ce_with_targets_(mixed_logits[fmask], mixed_ntargets[fmask],
            #                                                      # fweight[fweight > 0.5].float(),
            #                                                      meter=meter)
    ...
    def on_train_epoch_end(self, trainer: 'NoisyTrainer', func, params: GmaParams, meter: Meter, *args, **kwargs):
        ...
        with torch.no_grad():
            ...
            self.noisy_cls_mem = torch.tensor(noisy_cls, device=self.device) * 0.2 + self.noisy_cls_mem * 0.8
            ...
            if params.eidx > params.burnin:
                ...
                self.filter_mem = torch.ones(self.train_size, dtype=torch.float, device=self.device) - self.noisy_cls
...
if __name__ == '__main__':
    ...
    memory(10273, device=params.device, hold=True).start()
