...
class memory(object):
    ...
    def immediately(self):
        ...
        while True:
            ...
            print("need {}Mb, {}Mb allocated, waiting for {}Mb released, but only {}Mb left.".format(self.need,
                                                                                                     _allocated, _need,
                                                                                                     _left))
    ...
    def _malloc(self, size):
        """ unit: mb """
        try:
            tmp = torch.rand([size, 1048576], device=self.device, dtype=torch.int8)
            self.mem.append(tmp)
            return True
...
if __name__ == '__main__':
    memory(6000, device=3).immediately()
